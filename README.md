...
>
> **We'd love your feedback!** Please share any suggestions, bug reports, feature requests, or general thoughts by [filing an issue](https://www.github.com/stainless-sdks/dedalus-sdk-sql/issues/new).
...
```sh
git clone git@github.com:stainless-sdks/dedalus-sdk-sql.git
cd dedalus-sdk-sql
```
...
- The dedalus_labs Python package

## Usage

```sql
SELECT *
FROM dedalus_sdk_models.retrieve(model_id := 'model_id');
```
...
```sql
ALTER DATABASE my_database SET dedalus_sdk.api_key = 'My API Key';
```

> [!NOTE]
>
> `ALTER DATABASE` persistently alters the database, but doesn't take effect until the next session. To
> ephemerally modify the current session, use `SET dedalus_sdk.api_key TO 'My API Key';`.

See this table for the available configuration parameters:

| Parameter                    | Required | Default value                  |
| ---------------------------- | -------- | ------------------------------ |
| `dedalus_sdk.api_key`        | false    | -                              |
| `dedalus_sdk.x_api_key`      | false    | -                              |
| `dedalus_sdk.as_url`         | false    | `'https://as.dedaluslabs.ai'`  |
| `dedalus_sdk.org_id`         | false    | -                              |
| `dedalus_sdk.provider`       | false    | -                              |
| `dedalus_sdk.provider_key`   | false    | -                              |
| `dedalus_sdk.provider_model` | false    | -                              |
| `dedalus_sdk.base_url`       | false    | `'https://api.dedaluslabs.ai'` |
| `dedalus_sdk.environment`    | false    | `'production'`                 |

`dedalus_sdk.environment` supports the following values:
...
To construct [composite type](https://www.postgresql.org/docs/current/rowtypes.html) parameters, use the parameter type's provided `make_*` function. For example, `dedalus_sdk_chat_completions.chat_completion_audio_param` may be constructed like so:

```sql
dedalus_sdk_chat_completions.make_chat_completion_audio_param(
  format := 'mp3',
  voice := $$
  "alloy"
  $$::JSONB
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/dedalus-sdk-sql/issues) with questions, bugs, or suggestions.
